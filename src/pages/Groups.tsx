      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Select value={newGroup.courseId} onValueChange={v => { setNewGroup((g: any) => ({ ...g, courseId: v, levelId: "", instructorId: "" }));
              // تحديث المستويات مباشرة بدون useEffect
              const course = courses.find(c => c.id === v);
              setLevels(course?.levels || []);
              // تصفية المحاضرين للكورس
            ...
            <Input
              placeholder="مدة المحاضرة بالساعات (مثال: 1.5)"
              type="number"
              min={0.5}
              step={0.5}
            ...
            <div>
              <label>إضافة طلاب (بحث بالاسم أو الهاتف أو رقم الابليكيشن)</label>
                ...
                <ul className="list-disc ps-6">
                  {detailsDialog.group.studentIds?.length ?
                    detailsDialog.group.studentIds.map((sid: string) => {
                      ...
                      return st ? <li key={sid}>{st.name} ({st.phone})</li> : null;
                    }) : <li>لا يوجد طلاب</li>}
                </ul>
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-2 my-2 max-h-40 overflow-y-auto">
                  {students.filter(st => {
                    ...
                    <label key={st.id} className="flex items-center gap-1 border px-2 py-1 rounded cursor-pointer">
                      ...
                      />
                      {st.name} <span className="text-xs text-gray-500">({st.id} - {st.phone})</span>
                    </label>
                  ))}
                </div>
                <div className="mt-2">
                  <b>الطلاب المضافون للمجموعة:</b>
                  <ul className="list-disc ps-6">
